...
function initRsvp() {
    ...
    for (let i = 1; i <= (invitadoData ? invitadoData.boletosAsignados : 1); i++) {
    ...
    function irAPaso(numero) {
        ...
        const pasoNuevo = document.querySelector(`.rsvp-paso[data-paso="${numero}"]`);
        const direccion = (numero === 'exito' || (typeof numero === 'number' && typeof rsvpPasoActual === 'number' && numero > rsvpPasoActual)) ? 1 : -1;
    ...
    opciones.forEach(opcion => {
        opcion.addEventListener('click', () => {
            ...
            btnSiguiente.textContent = rsvpAsistencia === 'No' ? 'Enviar' : 'Siguiente';
        });
    });
    ...
    async function verificarConfirmacionReal(token, intentos = 5) {
        for (let i = 0; i < intentos; i++) {
            ...
            try {
                ...
                if (resultado.ok && (resultado.data.estado === 'Confirmado' || resultado.data.estado === 'Declinó')) {
                    return true;
                }
            } catch (e) {
                // seguimos intentando
    ...
    async function enviarRsvp() {
        ...
        const acompanantes = Array.from(acompanantesInputs).map(input => input.value);
        ...
        try {
            ...
            const confirmado = await verificarConfirmacionReal(token);

            if (confirmado) {
                document.getElementById('rsvp-exito-texto').textContent = rsvpAsistencia === 'Sí'
                    ? 'Tu asistencia ha sido confirmada. ¡Nos vemos el 4 de diciembre!'
                    : 'Gracias por avisarnos. Te extrañaremos ese día.';
                irAPaso('exito');
            } else {
                alert('No pudimos confirmar que tu respuesta se guardó correctamente. Por favor intenta de nuevo en unos momentos.');
                btnEnviar.disabled = false;
                btnEnviar.textContent = 'Confirmar';
            }
        } catch (error) {
            alert('No se pudo enviar tu confirmación. Verifica tu conexión e intenta de nuevo.');
            btnEnviar.disabled = false;
            btnEnviar.textContent = 'Confirmar';
        }
    }
    ...
    btnAtras.addEventListener('click', () => {
        if (typeof rsvpPasoActual === 'number' && rsvpPasoActual > 1) irAPaso(rsvpPasoActual - 1);
    });
